function fname = ct_data_gen_SBM_binary_fn(num)
    ...
    graph_model = "erdos_renyi"; % or PPM
    spread_model = "SIR"; % or SEINR
    if graph_model == "PPM"
        if N == 100
        ...
        if N == 100
            ...
            in_p = 0.002; % within-cluster edge probability
...
%     fname = sprintf( '../Data/ct_data_SBM_binary_SIR_N_%d_r_%d_in_p_%f_out_p_%f_K_%d_seed_%d.mat', N, r, in_p, out_p, K, seed);
        fname = sprintf(['../Data/%s/%d/%03d_ct_data_binary_%s_'...
        'N_%d_K_%d_'...
        'lambda_factor_%d_'...
        '_in_p_%f'...
        '_out_p_%f'...
        '_f_%d_k1_%d_k2_%d_outp_%f_' ...
        'seed_%d.mat'], ...
        spread_model, N, num, graph_model, N, K, lambda_coeff, in_p, out_p, f, k1, k2, outp, seed);
    fprintf("%s\n", fname);
%     save(fname);
end